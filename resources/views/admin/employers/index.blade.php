                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($employer->status == '0')
                                                <a class="badge badge-lg badge-danger text-white"
                                                    href="{{ route('employerToggle', [$employer->id]) }}">{{ __('Deactive') }}</a>
                                            @else
                                                <a class="badge badge-lg badge-success text-white"
                                                    href="{{ route('employerToggle', [$employer->id]) }}">{{ __('Active') }}</a>
                                            @endif
                                        ...
                                        <td style="width: 18%">
                                            <a class="btn btn-sm btn-info"
                                                href="{{ route('adminEditEmp', [$employer->id]) }}"><i
                                                    class="material-icons">edit</i></a>


                                            <button class="btn btn-sm btn-danger" data-toggle="modal"
                                                data-target="#employerDelete-{{ $employer->id }}" type="button"><i
                                                    class="material-icons">delete</i></button>

                                            <!-- Delete modal -->
                                            <div class="modal fade" id="employerDelete-{{ $employer->id }}"
                                                data-backdrop="static" data-keyboard="false" tabindex="-1"
                                                aria-labelledby="staticBackdropLabel-{{ $employer->id }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title text-center"
                                                                id="staticBackdropLabel-{{ $employer->id }}">Company Name:
                                                                {{ $employer->company->cname }}</h5>
